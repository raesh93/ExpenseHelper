...
Extracts tables from PDFs, merges tables with identical schemas, and sorts by date.
"""
...
class PDFTableExtractor:
    ...
    def _parse_transaction_table(self, table: List[List[str]]) -> pd.DataFrame:
        ...
        # The header should be in the first row
        # Expected columns: Date, SerNo., Transaction Details, (blank), (blank), Reward Points, Intl.# amount, Amount (in`)
        # But they may be split across multiple cells due to PDF formatting
        ...
        for row in table[1:]:
            ...
            # Reconstruct the transaction row
            # Expected format: Date | SerNo | Details | (blank) | (blank) | Points | (blank) | Amount
            if len(row) >= 3:
                try:
    ...
    def _is_transaction_table(self, df: pd.DataFrame) -> bool:
        """Check if a dataframe is a transaction table with proper columns."""
        # A transaction table should have columns like: Date, SerNo., Transaction Details, Amount
        cols = [str(col).lower().strip() for col in df.columns]
        
        # Check for required transaction columns
        has_date = any('date' in col for col in cols)
        has_serno = any('serno' in col or 'ser no' in col for col in cols)
        has_amount = any('amount' in col for col in cols)
        has_details = any('detail' in col or 'transaction' in col for col in cols)
        
        # A valid transaction table should have at least Date, SerNo, Details, and Amount
        return has_date and has_serno and has_amount and has_details
    
    @staticmethod
    def _table_to_dataframe(table: List[List[str]]) -> pd.DataFrame:
        """Convert a table (list of lists) to a DataFrame, handling multi-line headers."""
        ...
        # Clean headers - remove newlines and extra whitespace
        ...
        # Clean up whitespace in all cells
